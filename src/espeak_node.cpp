...
int synth_callback(short *wav, int num_samples, espeak_EVENT *events) {
    ...
    if (wav == nullptr || num_samples == 0) {
        if (debug_print) UtilityFunctions::print("[ESpeak C++] synth_callback: No audio data or 0 samples. num_samples: " + String::num_int64(num_samples));
        return 0; // Continue
    }

    if (debug_print) UtilityFunctions::print("[ESpeak C++] synth_callback: Received " + String::num_int64(num_samples) + " samples. Call count: " + String::num_int64(callback_call_count));
    ...
    if (byte_array) {
        ...
        if (debug_print) UtilityFunctions::print("[ESpeak C++] synth_callback: Total WAV data size: " + String::num_int64(byte_array->size()));
    } else {
        if (debug_print) UtilityFunctions::print("[ESpeak C++] synth_callback: byte_array is null!");
    }
